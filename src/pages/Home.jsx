...
const Home = () => {
    ...
    return (
        <>
            ...
            <section className="py-24 bg-white dark:bg-night reveal-on-scroll">
                <div className="container mx-auto px-4">
                    <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
                        <h2 className="text-4xl md:text-5xl font-serif text-brand-primary dark:text-white text-center md:text-left">Conoce FG</h2>
                        <Link to="/nosotros" className="text-brand-accent hover:text-brand-primary dark:hover:text-white transition-colors flex items-center gap-2 group">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-[minmax(200px,auto)]">
                        ...
                        <div className="md:col-span-5 bg-gray-100 dark:bg-night-card p-8 md:p-12 rounded-2xl flex flex-col justify-center hover:bg-white dark:hover:bg-gray-800 hover:shadow-xl dark:shadow-brand-accent/5 transition-all duration-300 border border-transparent hover:border-brand-accent/20 group">
                            ...
                            <h3 className="text-xl font-bold text-brand-primary dark:text-white mb-4">Visión</h3>
                            <p className="text-gray-600 dark:text-white leading-relaxed text-sm md:text-base">{t.home.intro.vision}</p>
                        </div>

                        {/* CLIENTE - Wide Block (Span 5) */}
                        <div className="md:col-span-12 lg:col-span-5 bg-brand-light dark:bg-night-card p-8 md:p-12 rounded-2xl flex flex-col justify-center relative overflow-hidden">
                            ...
                            <h3 className="text-xl font-bold text-brand-primary dark:text-white mb-4 flex items-center gap-2">
                                <span className="w-8 h-[1px] bg-brand-primary dark:bg-white"></span> El Cliente
                            </h3>
                            <p className="text-gray-600 dark:text-white leading-relaxed italic border-l-4 border-brand-accent pl-4">
                                {t.home.intro.cliente}
                            </p>
                        </div>

                        {/* SERVICIOS - Grid Block (Span 7) */}
                        <div className="md:col-span-12 lg:col-span-7 bg-white dark:bg-night-card border border-gray-100 dark:border-gray-800 p-8 md:p-12 rounded-2xl shadow-sm hover:shadow-lg transition-shadow duration-300 group">
                            <div className="flex justify-between items-center mb-8">
                                <h3 className="text-xl font-bold text-brand-primary dark:text-white">Nuestros Servicios</h3>
                                <Link to="/servicios" className="text-xs uppercase tracking-widest text-brand-primary/50 dark:text-white group-hover:text-brand-accent transition-colors">Ver todos</Link>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
                                {t.home.intro.servicios.map((servicio, i) => (
                                    <div key={i} className="flex items-center gap-3">
                                        <div className="w-1.5 h-1.5 bg-brand-accent rounded-full"></div>
                                        <span className="text-gray-600 dark:text-white text-sm">{servicio}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            {/* Team Section */}
            <section ref={teamRef} className="py-24 bg-white dark:bg-night reveal-on-scroll">
                <div className="container mx-auto px-4">
            ...
            {/* Process Section */}
            <section className="bg-brand-light dark:bg-night py-24 reveal-on-scroll">
                <div className="container mx-auto px-4">
                    <h2 className="text-4xl text-center font-serif mb-16 relative pb-6 after:content-[''] after:absolute after:bottom-0 after:left-1/2 after:-translate-x-1/2 after:w-16 after:h-px after:bg-brand-accent text-brand-primary dark:text-white">
                        {t.home.process.title}
                    </h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                        {t.home.process.steps.map((step, idx) => (
                            <div key={idx} className="bg-white dark:bg-night-card p-10 shadow-sm hover:shadow-xl transition-shadow duration-300 border-t-4 border-transparent hover:border-brand-accent">
                                <span className="text-6xl font-bold text-brand-primary dark:text-white block mb-6">{step.num}</span>
                                <h3 className="text-xl font-bold uppercase mb-4 text-brand-primary dark:text-brand-accent">{step.title}</h3>
            ...
            <section className="py-24 text-center container mx-auto px-4 reveal-on-scroll">
                <h2 className="text-4xl md:text-5xl font-serif mb-6 text-brand-primary dark:text-white">{t.home.cta.title}</h2>
                <p className="text-lg text-gray-600 dark:text-gray-400 mb-10 max-w-xl mx-auto">{t.home.cta.subtitle}</p>
                <a
                    href="https://wa.me/50660674833"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-block bg-brand-accent text-brand-primary px-10 py-4 text-xs font-bold uppercase tracking-[3px] hover:bg-brand-primary dark:hover:bg-white hover:text-white dark:hover:text-brand-primary transition-all duration-300 hover:-translate-y-1 shadow-lg"
                >
                    {t.home.cta.button}
                </a>
            </section>
